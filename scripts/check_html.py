...
class Page(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'meta' and (attrs.get('property') in ('og:image', 'og:video') or attrs.get('name') == 'twitter:image'):
            if attrs.get('content'):
                self.references.append(attrs['content'])
        attribute = 'href' if tag in ('a', 'area', 'link') else 'src'
        if tag in ('a', 'area', 'link', 'script', 'img', 'source', 'video', 'iframe'):
            value = attrs.get(attribute)
            if value is not None:
                self.references.append(value)


def check_site(directory, base_url, forbidden_hosts=(), allowed_urls=()):
    ...
    for path, page in pages.items():
        ...
        for reference in page.references:
            if reference.rstrip("/") in allowed_urls:
                continue
            ...
            if not destination.is_file():
                ...
                if fragment and not fragment.startswith(('gid=', '&gid=')) and fragment not in pages[destination].ids:
...
if __name__ == '__main__':
    ...
    count, errors = check_site(args.directory, args.base_url, args.forbid_host, args.allow_url)
